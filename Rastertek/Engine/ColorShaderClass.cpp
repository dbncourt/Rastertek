...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	return ColorShaderClass::InitializeShader(device, hwnd, L"Color.vs", L"Color.ps");
}
...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projecitonMatrix)
{
	//Set the shader parameters that it will use for rendering
	if (!ColorShaderClass::SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projecitonMatrix))
	{
		return false;
	}

	//Now render the prepared buffers with the shader
	ColorShaderClass::RenderShader(deviceContext, indexCount);
	return true;
}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, nullptr, nullptr, "ColorVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertexShaderBuffer, &errorMessage, nullptr);
	if (FAILED(result))
	{
		//If the shader failed to compile, it should have written something to the error message
		if (errorMessage)
		{
			ColorShaderClass::OutputShaderErrorMessage(errorMessage, hwnd, vsFilename);
		}
	...
	result = D3DX11CompileFromFile(psFilename, nullptr, nullptr, "ColorPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixelShaderBuffer, &errorMessage, nullptr);
	if (FAILED(result))
	{
		//If the shader failed to compile it should have written something to the error message
		if (errorMessage)
		{
			ColorShaderClass::OutputShaderErrorMessage(errorMessage, hwnd, psFilename);
		}
